...
// cache's incr, matching the write-only hincrby semantics).

import { eq, and, gte, sql, desc } from "drizzle-orm";
import { articles, feeds, userArticleStates, readingStats } from "../data/db/schema.js";
...
export class AnalyticsService {
  ...
  async trackFeedInteraction(
    ...
    try {
      const runtime = await getDataRuntime();
      await runtime.cache.incr(
        `analytics:feed-interactions:${userId}:${data.action}:${data.feedId}`,
      );
    } catch (error) {
      console.error("Failed to track feed interaction:", error);
    }
  }
